...
|-------------|-------------------|--------------------|
| Internal SRAM | 388884 | 520,192 |
| Flash Memory | 0 | varies |
| DMA Memory | 303024 | varies |

### คำถามวิเคราะห์ (ง่าย)

1. **Memory Types**: SRAM และ Flash Memory ใช้เก็บข้อมูลประเภทไหน?
SRAM (Static RAM)	ใช้เก็บ ข้อมูลชั่วคราว เช่น ตัวแปรที่อยู่ใน RAM, Stack, Heap และ Buffer ต่างๆ ขณะโปรแกรมกำลังทำงาน
Flash Memory	ใช้เก็บ ข้อมูลถาวร เช่น โปรแกรมที่ compile แล้ว (.bin หรือ .elf), ข้อมูลคงที่ (const), และไฟล์ SPIFFS / LittleFS
2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
Code / Text	Flash memory (เช่น 0x10000)	พื้นที่เก็บโค้ดของโปรแกรม
Global / Static variables	SRAM หรือ Flash (หากเป็น const)	ถ้าเป็น const จะอยู่ใน Flash
Heap (malloc, new)	SRAM (0x3FFAE6E0 ขึ้นไป)	สำหรับข้อมูลที่จัดสรรแบบ dynamic
Stack (ตัวแปรในฟังก์ชัน)	อยู่ใน SRAM เช่นเดียวกับ Heap แต่เติบโตจากทิศทางตรงข้าม	แต่ละ Task (ใน FreeRTOS) จะมี Stack ของตัวเอง
RTC Memory	0x50000000 – 0x50002000	ใช้สำหรับข้อมูลที่ไม่หายหลัง deep sleep (RTC Slow Memory)
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
SRAM (Data RAM)	~520 KB รวมทั้ง IRAM และ DRAM
Flash	ปกติ 4 MB (บางรุ่นมีมากกว่า)
RTC SRAM	8 KB (Slow) + 8 KB (Fast)
---
